...
// netlify/functions/connect-to-graphql/index.ts
var connect_to_graphql_exports = {};
__export(connect_to_graphql_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(connect_to_graphql_exports);
...
var handler = async (event) => {
  if (!process.env.GRAPHQL_ENDPOINT && !process.env.HASURA_ADMIN_SECRET) {
    return {
      ...
      headers,
      body: JSON.stringify({
        error: "GRAPHQL_ENDPOINT and HASURA_ADMIN_SECRET must be defined in env variables"
      })
    };
  }
